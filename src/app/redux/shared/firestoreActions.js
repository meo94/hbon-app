...
export const doListenToFirestoreCollection = (collection, node) =>
    (dispatch, getState) => {
        dispatch(doListenRequestedToFirestore(node, null));

        return firebaseService.fs.collection(collection)
            .get()
            .then(snap => {
                let items = {};
            ...
            .catch(error => dispatch(doListenRejectedToFirestore(node, error)));;
    }
...
export const doListenToMessages = () => doListenToFirestoreCollection('messages', 'messages'); 
